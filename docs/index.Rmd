...
app
```
By importing the cluster coordinate and reference spots previously selected, SpatialTime distance gradient can be calculated. For ```CoordMerge``` provide the path for coordinate folder and cluster name in ```pattern = ```. For example, using the cluster labeled as ```blastema```, it follows:
```sh
